...
| 设计 | 总架构和四份子规格已重写，状态 DRAFT，待用户复审 |
| 实施计划 | 未编写；等待规格批准 |
...
| Gate | 状态 | 原因 |
|---|---|---|
| G0 INPUT-FROZEN | `NOT_RUN` | specs 未批准，manifest/schema 未实现 |
...
用户复审总架构及四份子规格，选择批准、修改或拒绝。批准记录追加到决策台账后，才能调用
`writing-plans` 形成离线 Build Kit 的日期化实施计划。

在此之前，不创建真实 config/schema/Python/VBA 实现，不生成候选 CATVBA。
