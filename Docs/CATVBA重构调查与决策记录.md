...
>
> 当前阶段：重构路线、第一节“总体架构与包边界”、第二节“离线构建链”及首个里程碑范围 B 已确认；Core 运行时、目标机交付和同步治理仍待分段评审
...
|---|---:|---|---:|---|
| `CATIA_V5_SimpleMacroMenu.catvba` | 4,161,536 B | `B09195D5BF2787715CF4E8A50C840B0CE256A2408C83038149E1853E27906BA5` | 77 | 当前正式候选，但当前状态 NO-GO |
| `CAT_menu.catvba` | 4,007,424 B | `1C8EC220F3D97F84D5404F24225938B2A8E2821D3F16BFEF5964E97023CA2FCA` | 72 | 旧分叉产物，不能作为可信回滚 |

已确认的二进制事实：

- 正式候选内嵌的 77 个规范化源码模块与当前 `Src` 对应，已确认的源码缺陷已经进入二进制；
- 正式候选还包含没有对应文本模块的旧设计器 storage，说明它不是干净空工程重建结果；
...
5. 本地分支如何持续吸收 `origin/dev`，以及如何处理无共同历史的 `main`；
6. 完整设计文档、实施阶段、回滚点和任务级实施计划。
