...
> 当前 GitHub 远端分支仍停在 `037ab40696744678a57781d5197c152687520d84`，尚不包含第二周期
> implementation/delivery。新制品当前只能从本地工作副本取得；完成 fast-forward push 前，不能用 GitHub fresh clone 复刻现状。

## 1. 当前本地已签发制品

| 项目 | 固定值 |
|---|---|
| evidence commit / tree | `43274914149a67c3f076e5322e86060e3b1d1cc1` / `190494ac751fff8b51b925413f46b426f4e5eb25` |
| active bundle | `artifacts/b28-discovery/bundles/bundle-95bce28983237f5d84d0cbfd/` |
| provenance SHA-256 | `95bce28983237f5d84d0cbfd1aadc762f93948a84f16364359f2ebd002f170ba` |
...
bundle 还没有推送到 GitHub，因此不能从旧远端执行。只有最终 local clone、远端同步、GitHub fresh clone 与 Delivery doctor 全部通过后，
才可在批准的 blank VM、标准用户、原生 `cmd.exe` 中将 active bundle、`CURRENT.json` 和 fresh ledger 放入互不重叠的
本地 NTFS 目录，然后严格执行：

- `artifacts/b28-discovery/bundles/bundle-95bce28983237f5d84d0cbfd/QUICKSTART_B28.md`
- `artifacts/b28-discovery/bundles/bundle-95bce28983237f5d84d0cbfd/README_TARGET_B28.md`
- `artifacts/b28-discovery/bundles/bundle-95bce28983237f5d84d0cbfd/SECURITY_AND_REDACTION.md`
