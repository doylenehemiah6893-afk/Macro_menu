...
> **release:** `false`
> **Provenance:** `UNKNOWN` — 正文包含临时聊天转录、外部站点名称和未核验引用；不得推定可再分发
>
> 已发现的风险：
>
> - 代码会设置 `Document.CurrentFilter`，删除并重建过滤器，还会通过 `Selection.Search` 改变选择和显示状态；
> - 批处理示例会逐个打开、保存并关闭关联 Part，可能把过滤器和显示状态永久写回原文件；
> - 依赖 CATIA Document/Product/Part、Visualization Filter、Selection/Search、有效 `FullName` 和文件写权限，R2018 可用性未验证；
> - 多处 `On Error Resume Next` 吞错，没有完整文档类型校验、状态快照、失败回滚或安全的打开文档处理；
> - 正文是混合说明、聊天界面残片和代码片段，不是可直接导入、编译或审定的 VBA 模块。
